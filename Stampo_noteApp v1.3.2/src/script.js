...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchNotes(isArchived = false) {
    ...
    try {
      ...
      if (data.status === 'success') {
        console.log('Fetch data API berhasil!')
        allNotes = data.data
        renderNotes(allNotes)
      } else {
        noNotesMessage.classList.remove('hidden')
        noNotesMessage.textContent = 'Gagal memuat catatan.'
        console.error('Gagal memuat catatan:', data.message)
      }
    } catch (error) {
  ...
  async function handleNoteAdded(event) {
    ...
    try {
      ...
      if (result.status === 'success') {
        console.log('Catatan berhasil dibuat dengan ID:', result.data.noteId)
  ...
  async function handleArchiveNote(event) {
    ...
    try {
      ...
      const isArchived = noteToUpdate.archived
      const endpoint = isArchived
      ...
      if (result.status === 'success') {
        if (isArchived) {
          console.log('Catatan berhasil di-unarsip dengan ID:', noteId)
          showNotification('Berhasil Membatalkan Arsip!')
        } else {
          console.log('Catatan berhasil diarsipkan dengan ID:', noteId)
          showNotification('Catatan berhasil diarsipkan!')
        }
        fetchNotes(showingArchived)
      } else {
  ...
  async function handleDeleteNote(event) {
    ...
    if (confirmDelete) {
      ...
      try {
        ...
        if (result.status === 'success') {
          console.log('Catatan berhasil dihapus dengan ID:', noteId)
